...
def main(original_file_path, final_grade_files):
    ...
    for i, (question_number, file_path) in enumerate(final_grade_files.items(), start=1):
        ...
        #Thee filename willll be eg score_differences_IPC033-289 v1_289_quiz_graded_0_0.csv
        filename = f'score_differences_{question_number}_{file_path}'
...
def plot_3d_mse_mad(directory_path):
    ...
    ax_mad.set_xlabel('Text Edit Distance',fontsize=18, labelpad=10)
    ax_mad.set_ylabel('Tree Edit Distance', fontsize=18, labelpad=10)
    ax_mad.set_zlabel('MAD', rotation=90, labelpad=20, fontsize=18)
    ax_mad.tick_params(axis='z', labelsize=18, pad=10)
    plt.tight_layout()
...
"""final_grade_files_template = {
    'IPC033-289 v1': '289/quiz_graded_{0}_{1}.csv',
    'IPC033-291 v1': '290/quiz_graded_{0}_{1}.csv',
    'IPC033-291 v1': '291/quiz_graded_{0}_{1}.csv',
    'IPC033-292 v1': '292/quiz_graded_{0}_{1}.csv',
    'IPC033-293 v1': '293/quiz_graded_{0}_{1}.csv',
    'IPC033-294 v1': '294/quiz_graded_{0}_{1}.csv',
    'IPC033-295 v1': '295/quiz_graded_{0}_{1}.csv',
    'IPC033-309 v1': '309/quiz_graded_{0}_{1}.csv'
}"""
final_grade_files_template = {
    'IPC033-231 v1': '2024_april/231/quiz_graded_{0}_{1}.csv',
    'IPC033-232 v1': '2024_april/232/quiz_graded_{0}_{1}.csv',
    'IPC033-233 v1': '2024_april/233/quiz_graded_{0}_{1}.csv',
    'IPC033-234 v1': '2024_april/234/quiz_graded_{0}_{1}.csv',
    'IPC033-235 v1': '2024_april/235/quiz_graded_{0}_{1}.csv',
    'IPC033-236 v1': '2024_april/236/quiz_graded_{0}_{1}.csv',
    'IPC033-246 v1': '2024_april/246/quiz_graded_{0}_{1}.csv',
    'IPC033-253 v1': '2024_april/253/quiz_graded_{0}_{1}.csv'
}


for i in range(17):  # Iterates through 0 to 5 for the first number
    for j in range(16):  # Iterates through 0 to 5 for the second number
        # Generate the final_grade_files for the current iteration
        final_grade_files = {key: value.format(i, j) for key, value in final_grade_files_template.items()}
        
        # Generate textfile_name based on the current iteration
        csvfile_name_3 = f'multiple_differences__{i}_{j}.csv'

        # Generate png file name based on the current iteration
        png_file = f'multiple_grade_differences_violin__{i}_{j}.png'
        summary_file = f'summary_score_differences__{i}_{j}.csv'
